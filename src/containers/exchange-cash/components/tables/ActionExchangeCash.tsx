...
import { useExchangeCashContext } from '../..';
import UpdateExchangeCashModal from '../modals/UpdateExchangeCash';

const ActionExchangeCash = ({ row }: { row: IExchangeCash }) => {
  const { handleFetchApi } = useExchangeCashContext();
  ...
  const modalUpdateExchangeCashRef = useRef<FormDialogRef>(null);
  ...
  const openModalUpdateExchangeCashRef = () => {
    modalUpdateExchangeCashRef.current?.open();
  };

  const closeModalUpdateExchangeCashRef = () => {
    modalUpdateExchangeCashRef.current?.hide();
  };
  ...
  const handleEdit = () => {
    openModalUpdateExchangeCashRef?.();
    handleClose();
  };
  ...
  return (
    <>
      ...
      <UpdateExchangeCashModal
        modalUpdateExchangeCashRef={modalUpdateExchangeCashRef}
        closeModalUpdateExchangeCashRef={closeModalUpdateExchangeCashRef}
        row={row}
      />
    </>
  );
...
export default React.memo(ActionExchangeCash, equals);
