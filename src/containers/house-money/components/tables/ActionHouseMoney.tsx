...
import UpdateHouseMoneyModal from '../modals/UpdateHouseMoney';

const ActionHouseMoney = ({ row }: { row: IHouseMoney }) => {
  const { handleFetchApi } = useHouseMoneyContext();
  ...
  const modalUpdateHouseMoneyRef = useRef<FormDialogRef>(null);
  ...
  const openModalUpdateHouseMoneyRef = () => {
    modalUpdateHouseMoneyRef.current?.open();
  };

  const closeModalUpdateHouseMoneyRef = () => {
    modalUpdateHouseMoneyRef.current?.hide();
  };
  ...
  return (
    <>
      ...
      <UpdateHouseMoneyModal
        modalUpdateHouseMoneyRef={modalUpdateHouseMoneyRef}
        closeModalUpdateHouseMoneyRef={closeModalUpdateHouseMoneyRef}
        row={row}
      />
    </>
  );
...
export default React.memo(ActionHouseMoney, equals);
